<?php
function javenist_logo_shortcode( $atts ) {
	...
	if( isset($javenist_opt['logo_main']['url']) && $javenist_opt['logo_main']['url']!=''){
		$html .= '<div class="logo">';

			if($atts['logo_link']=='yes'){
				$html .= '<a href="'.esc_url( home_url( '/' ) ).'" title="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" rel="home">';
			}
				$html .= '<img src="'.esc_url($javenist_opt['logo_main']['url']).'" alt="'.esc_attr( get_bloginfo( 'name', 'display' ) ).'" />';
...
function javenist_mainmenu_shortcode( $atts ) {
	$javenist_opt = get_option( 'javenist_opt' );
	...
	<div class="main-menu-wrapper">
		<div class="visible-small mobile-menu"> 
			<div class="mbmenu-toggler"><?php echo esc_html($javenist_opt['mobile_menu_label']);?><span class="mbmenu-icon"><i class="ion-android-menu"></i></span></div>
			<div class="clearfix"></div>
			<?php wp_nav_menu( array( 'theme_location' => 'mobilemenu', 'container_class' => 'mobile-menu-container', 'menu_class' => 'nav-menu' ) ); ?>
		</div> 
		<div class="<?php if(isset($javenist_opt['sticky_header']) && $javenist_opt['sticky_header']) {echo 'header-sticky';} ?> <?php if ( is_admin_bar_showing() ) {echo 'with-admin-bar';} ?>">
...
function javenist_roadcategoriesmenu_shortcode ( $atts ) {
	...
	if(isset($javenist_opt['categories_menu_home']) && $javenist_opt['categories_menu_home']) {
		$cat_menu_class .=' show_home';
	}
	if(isset($javenist_opt['categories_menu_sub']) && $javenist_opt['categories_menu_sub']) {
		$cat_menu_class .=' show_inner';
	}
	?>
	<div class="categories-menu visible-large <?php echo esc_attr($cat_menu_class); ?>">
		<div class="catemenu-toggler"><i class="ion-android-menu"></i><span><?php if(isset($javenist_opt)) { echo esc_html($javenist_opt['categories_menu_label']); } else { _e('Category', 'javenist'); } ?></span></div>
		<?php wp_nav_menu( array( 'theme_location' => 'categories', 'container_class' => 'categories-menu-container', 'menu_class' => 'categories-menu' ) ); ?>
		<div class="morelesscate">
			<span class="morecate"><i class="ion-ios-more-outline"></i><?php if ( isset($javenist_opt['categories_more_label']) && $javenist_opt['categories_more_label']!='' ) { echo esc_html($javenist_opt['categories_more_label']); } else { _e('More Categories', 'javenist'); } ?></span>
			<span class="lesscate"><i class="ion-ios-more-outline"></i><?php if ( isset($javenist_opt['categories_less_label']) && $javenist_opt['categories_less_label']!='' ) { echo esc_html($javenist_opt['categories_less_label']); } else { _e('Close Menu', 'javenist'); } ?></span>
		</div>
	</div>
	<?php
...
function javenist_roadlangswitch_shortcode( $atts ) {
	$javenist_opt = get_option( 'javenist_opt' );
	...
	if (class_exists('SitePress')) { ?>
		<div class="switcher">
			<div class="language"><label><?php echo esc_html_e('language:','javenist')?></label><?php do_action('icl_language_selector'); ?></div> 
			<div class="currency"><label><?php echo esc_html_e('currency:','javenist')?></label><?php do_action('currency_switcher'); ?></div> 
		</div> 
	<?php }
...
function javenist_roadsocialicons_shortcode( $atts ) {
	$javenist_opt = get_option( 'javenist_opt' );
	...
	if(isset($javenist_opt['social_icons'])) {
		echo '<ul class="social-icons">';
		foreach($javenist_opt['social_icons'] as $key=>$value ) {
			if($value!=''){
				if($key=='vimeo'){
...
function javenist_roadminicart_shortcode( $atts ) {
...
function javenist_roadproductssearch_shortcode( $atts ) {
	...
	if( class_exists('WC_Widget_Product_Categories') && class_exists('WC_Widget_Product_Search') ) { ?>
		<div class="header-search"> 
			<div class="search-categories-container">
				<div class="cate-toggler"><?php esc_html_e('All Categories', 'javenist');?></div>
...
function javenist_brands_shortcode( $atts ) {
	global $javenist_opt;
	$brand_index = 0;
	
	if(isset($javenist_opt['brand_logos'])) {
		$brandfound = count($javenist_opt['brand_logos']);
	}
	$atts = shortcode_atts( array(
							'rowsnumber' => '1',
							'colsnumber' => '6',
	...
	if(isset($javenist_opt['brand_logos']) && $javenist_opt['brand_logos']) {
		$html .= '<div class="brands-carousel" data-col="'.$atts['colsnumber'].'">';
			foreach($javenist_opt['brand_logos'] as $brand) {
				if(is_ssl()){
...
function javenist_counter_shortcode( $atts ) {
	
	$atts = shortcode_atts( array(
							'image' => '',
							'number' => '100',
							'text' => 'Demo text',
							), $atts, 'javenist_counter' );
	$html = '';
	$html.='<div class="javenist-counter">';
...
function javenist_list_categories_shortcode( $atts ) {
...
function javenist_roadtimesale_shortcode( $atts ) {  
	$javenist_opt = get_option( 'javenist_opt' );
	...
	ob_start();
	if(isset($javenist_opt['sale-date-time'])) {
		echo '<div class="countdownsale" data-date="'.$javenist_opt['sale-date-time'].' 00:00:00" style="width: 470px; height: 117px;"></div>';
	}
	$html .= ob_get_contents();
...
function javenist_categoriescarousel_shortcode( $atts ) {
	global $javenist_opt;
	$categories_index = 0;
	if(isset($javenist_opt['cate_images'])){
		$categoriesfound = count($javenist_opt['cate_images']);
	}
	...
	if(isset($javenist_opt['cate_images'])){
		$html .= '<div class="categories-carousel" data-col="'.$atts['colsnumber'].'">';
			foreach($javenist_opt['cate_images'] as $categories) {
				if(is_ssl()){
...
function javenist_latestposts_shortcode( $atts ) {
	global $javenist_opt;
	$post_index = 0;
	$atts = shortcode_atts( array(
		'posts_per_page' => 5,
	...
	if($atts['image']=='wide'){
		$imagesize = 'javenist-post-thumbwide';
	} else {
		$imagesize = 'javenist-post-thumb';
	}
	$html = '';
			...
			foreach ( $postslist as $post ) {
				...
				$html.='<div class="item-col">';
					...
					if ( comments_open($post->ID) ) {
						if ( $num_comments == 0 ) {
							$comments = __('<span>0</span> comments', 'javenist');
						} elseif ( $num_comments > 1 ) {
							$comments = '<span>'.$num_comments .'</span>'. esc_html__(' comments', 'javenist');
						} else {
							$comments = __('<span>1</span> comment', 'javenist');
						}
						$write_comments = '<a href="' . get_comments_link($post->ID) .'">'.$comments.'</a>';
					}
					// Read more text
					if(!isset($javenist_opt['readmore_text'])){
						$javenist_opt['readmore_text'] = 'Read more';
					}
					...
					$html.='<div class="post-info">';    
						...
						$html.='<span class="post-author">';
							$html.= sprintf( wp_kses(__( '%s', 'javenist' ), array('a'=>array('href'=>array()))),'By <a href="'.$author_url.'">'.$author_name.'</a>' );
						$html.='</span>'; 
						$html.='<div class="post-excerpt">';
							$html.= Javenist_Class::javenist_excerpt_by_id($post, $length = $atts['length']);
						$html.='</div>'; 
						$html.='<div class="post-meta">';    
							...
							$html.='</div>'; 
						$html.='<a class="readmore" href="'.get_the_permalink($post->ID).'">'.'<span>' .esc_html($javenist_opt['readmore_text']). '</span>'.'</a>';
...
function javenist_contact_map( $atts ) {
	global $javenist_mapid;
	
	if(!isset($javenist_mapid)){
		$javenist_mapid = 1;
	} else {
		$javenist_mapid++;
	}
	...
	$html.='<div class="map-wrapper">';
		$html.='<div id="map'.$javenist_mapid.'" class="map" style="height: '.$map_height.'px"></div>';
	$html.='</div>';
	...
	<script type="text/javascript">
		jQuery(document).ready(function(){
			jQuery('#map<?php echo esc_attr($javenist_mapid);?>').gMap({
				scrollwheel: false,
...
function javenist_magnifier_options($att) {
